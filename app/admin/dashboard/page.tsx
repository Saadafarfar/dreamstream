      ...
      <div className="flex min-h-screen bg-[#030712] text-white">
        ...
        <main className="flex-1 flex items-center justify-center p-10">
          <div className="bg-red-500/10 border border-red-500/30 rounded-3xl p-8 max-w-md text-center">
            ...
            <button
              onClick={() => fetchOrders()}
              className="px-5 py-3 bg-red-500 hover:bg-red-600 rounded-xl font-semibold transition"
            >
              Try Again
            </button>
          </div>
      ...
      <main className="flex-1 p-6 md:p-10">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-2xl font-semibold transition disabled:opacity-50"
          >
            <RefreshCw
              size={18}
        ...
        {orders.length === 0 ? (
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center">
            ...
            <p className="text-slate-400">
              When new leads come in, they'll appear here.
            </p>
          </div>
        ) : (
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <KpiCard
                  label="Total Orders"
                  value={stats.total}
                  icon={<ShoppingCart size={28} className="text-blue-400" />}
                ...
                <KpiCard
                  ...
                  icon={<PhoneCall size={28} className="text-green-400" />}
                ...
                <KpiCard
                  label="Closed"
                  value={stats.closed}
                  icon={<CheckCircle2 size={28} className="text-red-400" />}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded-2xl pl-12 pr-5 py-4 text-white placeholder:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition"
                />
              </div>

              {/* Latest Orders */}
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                <h3 className="text-2xl font-bold mb-6">Latest Orders</h3>
                ...
                <div className="space-y-4">
                  {filteredOrders.length === 0 ? (
                    ...
                    filteredOrders.map((order) => (
                      <div
                        key={order.id}
                        className="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-800 hover:bg-slate-700 transition rounded-2xl p-4 gap-3"
                      >
                        ...
                        <div className="flex gap-2 flex-wrap">
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${
                              PLAN_COLORS[order.plan] || PLAN_COLORS.basic
                            }`}
                          >
                            {order.plan}
                          </span>
                    ...
                    <div key={order.id}>
                      <p className="text-sm">
                        <span className="font-semibold">
                          {order.full_name || "Someone"}
                        </span>{" "}
                        created a{" "}
                        <span className="text-blue-400">{order.plan}</span>{" "}
                        lead
                      </p>
...
function KpiCard({
  ...
  return (
    <div
      className={`bg-gradient-to-br ${gradient} border ${border} rounded-3xl p-5`}
    >
      <div className="flex justify-between items-start">
        <div className="min-w-0">
          <p className="text-slate-400 text-sm">{label}</p>
          <h2 className={`text-3xl md:text-4xl font-bold mt-2 ${valueColor}`}>
